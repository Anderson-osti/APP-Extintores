...
def cadastrar_empresa(nome_empresa, endereco, tipo_extintor, quantidade_extintor, capacidade_extintor, data_cadastro):
    ...
    try:
        empresa = {
            ...
            "data_cadastro": data_cadastro.isoformat()  # Convertendo para string
        }
...
def excluir_empresa(nome_empresa):
    ...
    try:
        result = db.empresas.delete_one({"nome_empresa": nome_empresa})
...
def gerar_pdf(empresas):
    class PDF(FPDF):
        def header(self):
            ...
            self.cell(0, 10, 'Relatório de Empresas Cadastradas', 0, 1, 'C')
            self.ln(10)
    ...
    pdf_file = "relatorio_empresas.pdf"
    pdf.output(pdf_file)
...
def tela_login():
    ...
    if st.button("Login"):
        if verificar_usuario(username, senha):
            ...
            st.session_state['username'] = username  # Armazenar usuário logado
...
def tela_relatorio():
    st.header("Gerar Relatório de Empresas")
    if st.button("Gerar Relatório"):
        empresas = listar_empresas()
        if empresas:
            gerar_pdf(empresas)
        else:
            st.warning("Nenhuma empresa cadastrada para gerar o relatório.")

def menu_principal():
    st.sidebar.title("Menu")
    opcao = st.sidebar.selectbox("Escolha uma opção", ["Cadastro de Empresa", "Listar Empresas", "Excluir Empresa", "Gerar Relatório", "Sair"])

    if opcao == "Cadastro de Empresa":
        tela_cadastro()
    elif opcao == "Listar Empresas":
        empresas = listar_empresas()
        if empresas:
            for empresa in empresas:
                st.write(f"Nome: {empresa['nome_empresa']}, Endereço: {empresa['endereco']}, Tipo: {empresa['tipo_extintor']}, Quantidade: {empresa['quantidade_extintor']}, Capacidade: {empresa['capacidade_extintor']}, Data de Cadastro: {empresa['data_cadastro']}")
        else:
